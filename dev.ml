...
let get_dir_if_mounted dev dir =
  ...
  >>= fun mount_lines -> 
  try Enum.find (Pcre.pmatch ~pat:("^" ^ dev)) mount_lines
      |> function 
      | <:re< [^" "]* 
              " on " 
              ( [^" "]{1+} as dir_existing ) 
  ...
  with Not_found -> 
    ( Msg.term `Notif "mount"
        [ "Device not already mounted; going to mount at '"; 
          dir; "'." ];
      Ok (`Mount dir) )
...
let mountpoint_fix_or_find dev dir = 
  ...
  >>= function
  | (`Dont_mount dir_existing) as dir_and_action -> Ok dir_and_action
...
let unmount ~settings = 
  ...
  | true  -> 
    (Sys.command 
       ("umount " ^ (s.mount_path |> Folder.escape_spaces))
     |> function 
     | 0 -> 
     ...
     | errcode -> 
       ( Msg.term `Error "unmount" 
           [ "Error occured during unmounting. Error-code was '"; 
